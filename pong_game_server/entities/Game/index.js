//you don't install webpack in the backend
//makethe code for the backend in ES5 syntax
...
Game.prototype.subscribe = function(socket, name, callback){
    ...
    //grab the array with all the key value pairs of the object PLAYERS
    if(Object.keys(PLAYERS).length === 2){
        this.addSpectator(socket, name, callback);
        return callback(false, 'Spectator Added');
    }
    //check if there are two players and if there is a player on the left, put the player on the right when adding new player
    else{
        var side = Object.keys(PLAYERS).length === 0 ? 
        'left' :
        PLAYERS[Object.keys(PLAYERS)[0]].side === 'left' ?
        'right' : 'left';
