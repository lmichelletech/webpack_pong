...
Player.prototype.update = function (dimensions){
    ...
    this.position.y = Math.max(Math.min(this.position.y +
    this.velocity.y, dimensions.height -
    this.dimensions.height), 0);

    if(this.side === 'left'){
        this.position.x = Math.max(Math.min(this.position.x
        +this.velocity.x, dimensions.width / 2 -
    this.dimensions.width), 0);
    }

    if(this.side === 'right'){
        this.position.x = Math.min(Math.max
        (dimensions.width / 2, this.position.x +
        this.velocity.x), dimensions.width - 
